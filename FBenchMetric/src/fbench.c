...
static double pi = pic, twopi = pic * 2.0, piover4 = pic / 4.0, fouroverpi = 4.0
      / pic, piover2 = pic / 2.0;

/*  Coefficients for ATAN evaluation  */

static double atanc[] =
{ 0.0, 0.4636476090008061165, 0.7853981633974483094, 0.98279372324732906714,
      1.1071487177940905022, 1.1902899496825317322, 1.2490457723982544262,
      1.2924966677897852673, 1.3258176636680324644 };
   ...
   double x;
{
   ...
   atl: z = x * x;
   b = ((((893025.0 * z + 49116375.0) * z + 425675250.0) * z + 1277025750.0) * z
         + 1550674125.0) * z + 654729075.0;
...
//	fgets(tbfr, sizeof tbfr, stdin);
   ...
   double startNanoSec = t.tv_nsec;
   double startSec = t.tv_sec;
   
   printf("Start time is %f sec, %f nano sec\n", startSec, startNanoSec);
//	struct timeval t;
//	int retval = gettimeofday( &t, NULL );
#endif
   ...
   for (itercount = 0; itercount < niter; itercount++)
   {
      ...
      aberr_osc = 1.0
            - (od_sa[1][0] * od_sa[1][1]) / (sin(od_sa[0][1]) * od_sa[0][0]);
...
//	fgets(tbfr, sizeof tbfr, stdin);
   ...
   double stopNanoSec = t.tv_nsec;
   double stopSec = t.tv_sec;
   printf("Stop  time is %f sec, %f nano sec\n", stopSec, stopNanoSec);
  
   double totalSecs = ((stopSec - startSec) * 1000000000) + (stopNanoSec - startNanoSec);
   
   printf("Total nano seconds:  %f\n" , totalSecs);
   double normalizedNanoSeconds = 0; 
   normalizedNanoSeconds = totalSecs/10000.0;
   printf("Normalization of nano seconds: %f\n", normalizedNanoSeconds);
   double seconds = normalizedNanoSeconds/1000000000.0;
   printf("Seconds: %f\n", seconds);
