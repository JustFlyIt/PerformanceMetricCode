...
#define CACHE_MAX_BITS (23) /* 1<<bits is largest cache in bytes */
...
static long logmemsize = CACHE_MAX_BITS, memsize = 1L << CACHE_MAX_BITS,
      repeat_count = REPEAT_COUNT;
static long resolution = RESOLUTION, timeslots, duration = MEASURE_TIME, type =
      NOCALIBRATE;
...
{
   /* Force results to flush quicker. */
   return isatty(fd);
}
      ...
      double tmicrosec, long datasize)
{
   if (tmicrosec > 0.0)
   {
      ARRAY(times,i,j,repeat_count) = ((tmicrosec * 1000.0)
            - (refcnt * overhead_per_ref)) / refcnt;

      ARRAY(bws,i,j,repeat_count) = ((refcnt * datasize) / (1024.0 * 1024.0))
   ...
   if (i == 0)
   {
      ...
      else
      {
         ...
         ARRAY(times,i,j,repeat_count) / ARRAY(times, i, j - 1, repeat_count);
      }
   }
   else
   {
   ...
   if (my_isatty(1))
   {
      printf("%-15ld %-15.2f %-15.2f %-15.2f\n", sizes[i],
            ARRAY(times, i, j, repeat_count), ARRAY(bws, i, j, repeat_count),
            ARRAY(percents, i, j, repeat_count));
   }
   else
...
void compute_cache_sizes(double *times, double *bws, double *percents,
      long *sizes)
{
   ...
   {
      printf("Level 1 Cache: %ld bytes\n", cachea);
      if ((times[maxa] / times[maxb] < THRESHOLD)
            && (1.0 / percents[maxb] < THRESHOLD))
...
long usage(long argc, char **argv)
{
   ...
   if ((type & (READONLY | WRITEONLY | READWRITE | MEMORYCOPY | MEMORYSET))
         == 0)
   {
      if (my_isatty(1))
         type |= READONLY|WRITEONLY|READWRITE|MEMORYCOPY|MEMORYSET; 
         //type |= READONLY | WRITEONLY | READWRITE | MEMORYCOPY | MEMORYSET | HANDTUNED;
      else
         type |= READWRITE;
   }
...
void fake_out_optimizations(DATATYPE *x, long bytes)
{
   ...
   //fd2 = open("/dev/null",O_WRONLY, 777);

   if (fd == -1)
      fd = open("/dev/null", O_WRONLY, 777);

   //assert(fd=open("/dev/null",O_WRONLY, 777));

   assert(
         write(fd,(void *)x,bytes>sizeof(DATATYPE)?sizeof(DATATYPE):bytes)!=-1);
}
...
double benchmark_cache_ronly(REGISTER DATATYPE *x, REGISTER long limit,
      long *oloops, double *ous)
{
   REGISTER long index = 0, loops = 0;
...
double hand_benchmark_cache_ronly(REGISTER DATATYPE *x, REGISTER long limit,
      long *oloops, double *ous)
{
   REGISTER long index = 0, loops = 0;
   REGISTER DATATYPE sum = (DATATYPE) 0;
...
double benchmark_cache_wonly(REGISTER DATATYPE *x, REGISTER long limit,
      long *oloops, double *ous)
{
   REGISTER long index = 0, loops = 0;
...
double hand_benchmark_cache_wonly(REGISTER DATATYPE *x, REGISTER long limit,
      long *oloops, double *ous)
{
   REGISTER long index = 0, loops = 0;
   REGISTER DATATYPE wval = (DATATYPE) 0xf;
...
double benchmark_cache(REGISTER DATATYPE *x, REGISTER long limit, long *oloops,
      double *ous)
{
   REGISTER long index = 0, loops = 0;
...
double hand_benchmark_cache(REGISTER DATATYPE *x, REGISTER long limit,
      long *oloops, double *ous)
{
   REGISTER long index = 0, loops = 0;
...
double benchmark_cache_memory_copy(REGISTER void *x, REGISTER void *y,
      REGISTER long bytes, long *oloops, double *ous)
{
...
double benchmark_cache_memory_set(REGISTER void *x, REGISTER long bytes,
      long *oloops, double *ous)
{
   REGISTER long loops = 0;
   ...
   for (j = 0; j < timeslots; j += resolution)
   {
      ...
      for (i = 1; i < resolution; i++)
      {
         if (j + i < timeslots)
         {
            ...
            sizes[j + i] = sizes[j + i]
                  - sizes[j + i] % (long) sizeof(DATATYPE);
...
void do_memory_copy(long *sizes, void *x, double *times, double *bws,
      double *percents)
{
   ...
   for (i = 0; i < timeslots; i++)
   {
      ...
      {
         refcnt = benchmark_cache_memory_copy(x, y, limit, &tloops, &tmicrosec)
               * 2.0;
         ...
         compute_stats(i, j, refcnt, overhead_per_ref, times, bws, percents,
               sizes, tmicrosec, 1);
      }
   }
...
void do_memory_set(long *sizes, DATATYPE *x, double *times, double *bws,
      double *percents)
{
   long limit, j, i, tloops;
   double refcnt, overhead_per_ref = 0.0, tmicrosec;
...
void do_read_only(long *sizes, DATATYPE *x, double *times, double *bws,
      double *percents)
{
   long limit, j, i, tloops;
   double refcnt, overhead_per_ref = 0.0, tmicrosec;
   ...
   for (i = 0; i < timeslots; i++)
   {
      ...
      for (j = 0; j < repeat_count; j++)
      {
         ...
         compute_stats(i, j, refcnt, overhead_per_ref, times, bws, percents,
               sizes, tmicrosec, sizeof(DATATYPE));
      }
   }
...
void do_write_only(long *sizes, DATATYPE *x, double *times, double *bws,
      double *percents)
{
   long limit, j, i, tloops;
   double refcnt, overhead_per_ref = 0.0, tmicrosec;
...
void do_read_write(long *sizes, DATATYPE *x, double *times, double *bws,
      double *percents)
{
   long limit, j, i, tloops;
   double refcnt, overhead_per_ref = 0.0, tmicrosec;
...
int main(int argc, char **argv)
{
   ...
   assert(
         percents = (double * )malloc(
               timeslots * repeat_count * sizeof(double)));
